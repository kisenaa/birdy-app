...
import { predictBirdClassification, predictBirdDetection, uploadImageToSpaceClassification, uploadImageToSpaceDetection } from "@/services/api/gradioAPI"
...
export const Home: FC<DashboardTabScreenProps<"Home">> = function Home(_props) {
  ...
  const test_nativeModule = async () => {
    ...
    const tempArr = new Uint32Array([1, 2, 3, 4, 5])
    const t3 = performance.now()
    console.log(await BirdDetector.detect(tempArr.buffer))
  ...
  const fetchBirdChatbot = async () => {
    const prompt = `bird species of ${inferResult?.label}`
    try {
  ...
  const onPress = () => {
    const options = ["Camera App", "Gallery", "Cancel"]
      ...
      {inferResult !== null && (
        <>
          ...
          >
            ...
            <Button text="Explain more about the bird !" onPress={fetchBirdChatbot} style={{ width: "100%", marginTop: 8, marginBottom: 12, borderRadius: 16 }} preset="filled" />

            {chatbotResponse
              ?.split("\\n")
              .filter((paragraph) => paragraph.trim() !== "") // Skip empty or whitespace-only
              .map((paragraph, index) => (
                <Text
                  key={index}
                  size="md"
